...
def parse_arguments():
    ...
    parser.add_argument(
        '-w', '--workers',
        help='Number of worker threads for concurrent scanning',
        type=int,
        default=10
    )
    ...
    if not args.url and not args.list:
        parser.error("Either -u/--url or -l/--list must be provided")
...
def scan_single_url(url, args):
    ...
    for tech_name, tech_info in technologies.items():
        ...
        scan_output = run_nuclei_scan(url, tech_name, version)
        if scan_output:
                    ...
                    try:
                        ...
                        if enriched_finding['vulners']:
                            ...
                            if vuln_info.get('references', []):
                                print("    References:")
                                for ref in vuln_info['references'][:3]:
                                    print(f"      - {ref}")
...
def scan_from_list(filename, args):
    """
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            future_to_url = {executor.submit(scan_single_url, subdomain, args): subdomain for subdomain in subdomains}
...
def main():
    ...
    if args.list:
        results = scan_from_list(args.list, args)
    else:
        results = scan_single_url(args.url, args)
...
if __name__ == "__main__":
    print(f"{Fore.MAGENTA}autox{Style.RESET_ALL}")
    main()
